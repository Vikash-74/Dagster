...
from dagster_webserver.user import User,UserList
...
class GraphQLServer(ABC, Generic[TRequestContext]):
    def __init__(self, app_path_prefix: str = ""):
        ...
        # Adding dummy users, not gonna happen in real-life :)
        self._user_list.add(User(identifier=1, username='admin', password='DagsterPa$$word'))
        self._user_list.add(User(identifier=2, username='readuser', password='re@dUserPassword'))
        self._user_list.add(User(identifier=3, username='monitoringuser', password='monitorPaSSword'))
        self._login_manager = LoginManager(redirect_to='login', secret_key='secret')
        # Suggestion: randomly generate the secret key everytime the server starts.
        self._login_manager.set_user_loader(self._user_list.user_loader)
    ...
    def create_asgi_app(
        ...
        **kwargs,
        ...
        app = Starlette(
            ...
            middleware=self.build_middleware() + [
                    ...
                    Middleware(
                        ...
                        backend=SessionAuthBackend(self._login_manager),
        ...
        app.state.login_manager = self._login_manager
        return app
