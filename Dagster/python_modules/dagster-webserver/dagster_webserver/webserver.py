...
from .db_auth import authenticate_standard_user, create_standard_user
...
class DagsterWebserver(
    ...
    def make_request_context(self, conn: HTTPConnection) -> BaseWorkspaceRequestContext:
        ctx = self._process_context.create_request_context(conn)
        ctx.request = conn # <-- ADD THIS LINE
        return ctx
    ...
    async def login_page(self, request: Request):
        ...
        if request.query_params.get("success") == "true":
            ...
            success_html = f'<div class="success">{success_message}</div>'

        if request.user.is_authenticated:
            return self.index_html_endpoint(request)
        ...
        if request.method == "POST":
            ...
            auth_method = data.get("auth_method", "standard") # Default to standard if not specified
            ...
            if not username or not password:
                ...
                if auth_method == "standard":
                    ...
                    user_from_list = self._user_list.get_by_username(username)

                    if user_from_list:
                        if user_from_list.check_password(password):
                            user_details = user_from_list
                        else:
                            user_details = None
                    else:
                        user_details = authenticate_standard_user(username, password)

                    if not user_details:
                        error_message = "Invalid username or password (Standard)."
                        error_html = f'<div class="error">{error_message}</div>'
                    else:
                        await login_user(request, user_details)
                        return self.index_html_endpoint(request)
                elif auth_method == "ldap":
                    # --- LDAP AUTHENTICATION LOGIC ---
                    user_details = authenticate_ldap_user(username, password) 
                    if not user_details:
                        error_message = "Invalid username or password (LDAP)."
                        error_html = f'<div class="error">{error_message}</div>'
                    else:
                        await login_user(request,user_details)
                        return self.index_html_endpoint(request)
        ...
        login_path = self.relative_path("webapp/build/login.html")
        try:
            with open(login_path, encoding="utf8") as f:
                # Read the Templace and replace the placeholder
                content = (
                    f.read()
                    .replace("{{ERROR_HTML_PLACEHOLDER}}", error_html)
                    .replace("{{SUCCESS_HTML_PLACEHOLDER}}", success_html)
                )
                return HTMLResponse(content)
        except FileNotFoundError:
            return PlainTextResponse("Login page template not found.", status_code=500)
    ...
    async def signup_page(self, request: Request):
        ...
        if request.method == "POST":
            ...
            print(f"--- SIGNUP ATTEMPT (TESTING) ---")
            ...
            if not all([username, password, confirm_password]):
                ...
                success, message = create_standard_user(username, password)
                if success:
                    return RedirectResponse(url="/login?success=true", status_code=302)
            
            if 'error_message' in locals() and error_message:
                error_html = f'<div class="error">{error_message}</div>'

        signup_path = self.relative_path("webapp/build/signup.html")
        try:
            with open(signup_path, encoding="utf8") as f:
                content = f.read().replace("{{ERROR_HTML_PLACEHOLDER}}", error_html)
                return HTMLResponse(content)
        except FileNotFoundError:
            return PlainTextResponse("Signup page template not found.", status_code=500)

    async def logout_page(self, request: Request): # Ensure this function is present for logout
        await logout_user(request) # Starlette-Login utility
    ...
    def build_routes(self):
        routes = (
            [
                Route('/login', self.login_page, methods=['GET', 'POST'], name='login'),
                Route('/signup', self.signup_page, methods=['GET', 'POST'], name='signup'),
                Route('/logout', self.logout_page, name='logout'),
